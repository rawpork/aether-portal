...
- [x] **Card & Drawer Stacking Fix:** The cluster drawer measured 374px instead of 340px (padding and border were added on top of its width), so it covered the node card's right edge and half-hid the close button at every desktop width; at 768-800px the card also slid off the left edge. The drawer is now 340px including padding, the card sits 16px from it and narrows on small desktops instead of leaving the screen, the card stacks above neighbouring panels (z-index 20, below the top bar so its Filter/Settings menus stay on top), the close and delete buttons have a fixed 12px top-right inset on their own layer, and list/grid/timeline and drawer cards each get position: relative + isolation: isolate.
...
- [x] **Filter Toolbar Cleanup:** The platform pills are now one segmented control (All, YouTube, X/Twitter, Facebook, Links, Notes, Images with live counts) in a single 32px row under the header, and the header's separate Filter dropdown is gone, so All, Links, Notes and Images no longer appear twice. A "Filters" popover (gear icon) at the right end of that row holds Time, Category (a select, so Dev Tasks and Videos stay filterable), Category/Rainbow colors and Hide Unlinked, plus Clear All Filters; a badge shows how many of those filters are on. On phones the segmented control swipes sideways and the popover spans the screen. From 601-900px the Telegram and view buttons go icon-only, fixing a top bar that overflowed (and squeezed search to 26px) after the Board button was added.
